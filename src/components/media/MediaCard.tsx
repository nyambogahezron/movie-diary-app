...
import { Film, Tv, Star, Clock, Eye } from 'lucide-react';
...
const MediaCard: React.FC<MediaCardProps> = ({ media }) => {
  const detailsPath = media.type === 'movie' ? `/movie/${media.id}` : `/tv/${media.id}`;
        ...
        {media.status && (
          <div className="mt-2">
            <span className={`px-2 py-1 text-xs rounded-full ${
              media.status === 'Currently Watching' ? 'bg-blue-900 text-blue-200' : 
              media.status === 'Completed' ? 'bg-green-900 text-green-200' :
              media.status === 'Planning to Watch' ? 'bg-purple-900 text-purple-200' :
              media.status === 'Paused' ? 'bg-yellow-900 text-yellow-200' :
        ...
        <div className="mt-auto pt-3 flex justify-between items-center text-sm">
          {media.rewatches > 0 && (
            <div className="flex items-center text-gray-400">
              ...
              <span>{media.rewatches === 1 ? 'Watched once' : `Watched ${media.rewatches} times`}</span>
            </div>
          )}
